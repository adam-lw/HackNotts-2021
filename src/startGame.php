...
session_start();
$code = $_POST['code'];
mysqli_query($conn, "UPDATE GameSessions SET State = 2, Countdown = 30, Question = 1 WHERE Code = '$code' ");
    ...
    switch($qnum) { // Depending on the question chosen, run some different code
        ...
        case 3: // "Which of these songs has <player> listened to the most?";
            ...
            if (mysqli_num_rows($tracks)>15) {
                ...
                // insertQuestion($n, $t, $q1, $a1, $a2, $a3, $a4, $correct, $url)
                insertQuestion($conn, $code, $inserted+1, "PLAYER", $tempQuestion, $a1, $a2, $a3, $a4, $correctAnswer, $urls[rand(1,3)], $topTrackIDs[0]);
                $inserted = $inserted + 1;
            }
            break; 
